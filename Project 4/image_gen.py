...
for idx, fname in enumerate(images):
    ...
    left_line = np.array(list(zip(np.concatenate((left_fitx, left_fitx[::-1]), axis=0), np.concatenate((yvals, yvals[::-1]), axis=0))), np.int32)
    right_line = np.array(list(zip(np.concatenate((right_fitx, right_fitx[::-1]), axis=0), np.concatenate((yvals, yvals[::-1]), axis=0))), np.int32)
